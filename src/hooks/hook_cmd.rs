...
mod tests {
    ...
    #[test]
    fn test_codex_json_output_structure_allows_rewrite() {
        let result = run_codex_inner(&codex_input("git status")).unwrap();
        let v: Value = serde_json::from_str(&result).unwrap();
    ...
    #[test]
    fn test_codex_event_and_command_validation() {
        let mut input: Value = serde_json::from_str(&codex_input("git status")).unwrap();
        input["hook_event_name"] = json!(PRE_TOOL_USE_KEY);
        assert!(run_codex_inner(&input.to_string()).is_some());
        for event in [json!("PostToolUse"), json!(null), json!(42)] {
            input["hook_event_name"] = event;
            assert!(run_codex_inner(&input.to_string()).is_none());
        }
        input["hook_event_name"] = json!(PRE_TOOL_USE_KEY);
        for tool_input in [json!({}), json!({"command": ""}), json!({"command": 42})] {
    ...
    #[test]
    fn test_codex_bom_prefixed_input() {
        let input = codex_input("git status");
        assert_eq!(
            run_codex_inner(&format!("\u{feff}\u{feff}{input}")),
            run_codex_inner(&input)
        );
    }
