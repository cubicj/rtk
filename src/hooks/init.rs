...
fn uninstall_codex_at(codex_dir: &Path, ctx: InitContext) -> Result<Vec<String>> {
    ...
    removed.extend(remove_codex_legacy_files(codex_dir, ctx)?);
...
fn run_codex_mode(global: bool, patch_mode: PatchMode, ctx: InitContext) -> Result<()> {
    let (hooks_json_path, legacy_rtk_md_path) = if global {
        let codex_dir = resolve_codex_dir()?;
        (codex_dir.join(HOOKS_JSON), codex_dir.join(RTK_MD))
    } else {
        let codex_dir = PathBuf::from(CODEX_DIR);
        (codex_dir.join(HOOKS_JSON), codex_dir.join(RTK_MD))
    };

    run_codex_mode_with_paths(hooks_json_path, legacy_rtk_md_path, global, patch_mode, ctx)
}
...
fn run_codex_mode_with_paths(
    ...
    let legacy_dir = legacy_rtk_md_path
        .parent()
        .context("RTK.md path missing parent directory")?;
    let removed_legacy = remove_codex_legacy_files(legacy_dir, ctx)?;
    ...
    if !dry_run {
        ...
        match patch_result {
            PatchResult::Patched => println!("  hooks.json: RTK PreToolUse entry added"),
            PatchResult::AlreadyPresent => {
                println!("  hooks.json: RTK PreToolUse entry already present")
            }
            PatchResult::Skipped => println!("  hooks.json: not changed"),
            PatchResult::WouldPatch | PatchResult::Declined => {}
        }
        for item in removed_legacy {
            println!("  legacy cleanup: {}", item);
        }
        if global {
            println!("\n  Codex global hook path: {}", hooks_json_path.display());
        } else {
            println!("\n  Codex project hook path: {}", hooks_json_path.display());
        }
    }
...
fn print_codex_manual_instructions(path: &Path) {
    println!("\nCodex hooks.json patch skipped.\n");
    println!("Add this entry to {}:", path.display());
    println!(
        r#"{{"hooks":{{"PreToolUse":[{{"matcher":"Bash","hooks":[{{"type":"command","command":"{}","timeout":30}}]}}]}}}}"#,
        CODEX_HOOK_COMMAND
    );
}
...
fn is_codex_hook_command(cmd: &str) -> bool {
    cmd == CODEX_HOOK_COMMAND
        || cmd.ends_with("/rtk hook codex")
        || cmd.ends_with("\\rtk.exe hook codex")
}
...
fn remove_codex_legacy_files(codex_dir: &Path, ctx: InitContext) -> Result<Vec<String>> {
    let mut removed = Vec::new();
    ...
    if agents_md_path.exists() {
        ...
        let mut content = fs::read_to_string(&agents_md_path)
        ...
        if has_rtk_reference(&content, &[RTK_MD_REF, absolute_rtk_md_ref.as_str()]) {
            let cleaned = content
                .lines()
                .filter(|line| {
                    let trimmed = line.trim();
                    trimmed != RTK_MD_REF && trimmed != absolute_rtk_md_ref
                })
                .collect::<Vec<_>>()
                .join("\n");
            content = clean_double_blanks(&cleaned);
            changed = true;
            removed.push("AGENTS.md: removed RTK.md reference".to_string());
        }

        if changed && !ctx.dry_run {
            atomic_write(&agents_md_path, &content).with_context(|| {
...
fn show_claude_config() -> Result<()> {
    ...
    println!("  rtk init --codex            # Configure local .codex/hooks.json");
    println!(
        "  rtk init -g --codex         # Configure $CODEX_HOME/hooks.json (or ~/.codex/hooks.json)"
    );
...
fn show_codex_config() -> Result<()> {
    ...
    if codex_hook_registered_in(&global_hooks_json)? {
        println!("[ok] Global hooks.json: {}", CODEX_HOOK_COMMAND);
        println!("     {}", global_hooks_json.display());
    } else if global_hooks_json.exists() {
        println!(
            "[--] Global hooks.json: exists but RTK hook not registered ({})",
    ...
    if codex_hook_registered_in(&local_hooks_json)? {
        println!("[ok] Local hooks.json: {}", CODEX_HOOK_COMMAND);
        println!("     {}", local_hooks_json.display());
    } else if local_hooks_json.exists() {
        println!(
            "[--] Local hooks.json: exists but RTK hook not registered ({})",
    ...
    println!("\nUsage:");
    println!("  rtk init --codex                 # Configure local .codex/hooks.json");
    println!("  rtk init -g --codex              # Configure $CODEX_HOME/hooks.json (or ~/.codex/hooks.json)");
    println!("  rtk init --codex --uninstall     # Remove local Codex RTK hook");
    println!("  rtk init -g --codex --uninstall  # Remove global Codex RTK hook");

    Ok(())
}

fn codex_hook_registered_in(path: &Path) -> Result<bool> {
    if !path.exists() {
        return Ok(false);
    }
    let content =
    ...
    let root: serde_json::Value = serde_json::from_str(&content)
        .with_context(|| format!("Failed to parse {} as JSON", path.display()))?;
    Ok(codex_hook_already_present(&root))
}
...
mod tests {
    ...
    #[test]
    fn test_run_codex_mode_global_installs_hooks_json_only() {
        let temp = TempDir::new().unwrap();
        let hooks_json = temp.path().join("hooks.json");
        let legacy_rtk_md = temp.path().join("RTK.md");
        let legacy_agents_md = temp.path().join("AGENTS.md");
        ...
        assert!(content.contains("rtk hook codex"));
        assert!(!legacy_rtk_md.exists());
        assert!(!legacy_agents_md.exists());
    }
    ...
    #[test]
    fn test_insert_codex_hook_entry_installs_pre_tool_use_hook() {
        ...
        assert_eq!(entries[0]["hooks"][0]["command"], "rtk hook codex");
    }

    #[test]
    fn test_codex_hook_already_present_matches_absolute_command() {
        let root = serde_json::json!({
            "hooks": {
                "PreToolUse": [{
                    ...
                    "hooks": [{
                        "type": "command",
                        "command": "/home/cubicj/.cargo/bin/rtk hook codex",
                        "commandWindows": "C:\\Users\\CubicJ\\.cargo\\bin\\rtk.exe hook codex"
                    }]
                }]
            }
        });
