...
pub(crate) mod tests {
    ...
    #[cfg(windows)]
    fn command_stdout(text: &str) -> Command {
        let mut cmd = Command::new("cmd.exe");
    ...
    fn command_stderr(text: &str) -> Command {
        let mut cmd = Command::new("cmd.exe");
        cmd.arg("/C").arg(format!("echo {text} 1>&2"));
        cmd
    }
    ...
    fn command_stderr(text: &str) -> Command {
        let mut cmd = Command::new("sh");
        cmd.arg("-c").arg(format!("echo {text} >&2"));
        cmd
    }
    ...
    fn command_combined(stdout_text: &str, stderr_text: &str) -> Command {
        let mut cmd = Command::new("cmd.exe");
        cmd.arg("/C")
            .arg(format!("echo {stdout_text} & echo {stderr_text} 1>&2"));
        cmd
    }
    ...
    fn command_combined(stdout_text: &str, stderr_text: &str) -> Command {
        let mut cmd = Command::new("sh");
        cmd.arg("-c")
            .arg(format!("echo {stdout_text}; echo {stderr_text} >&2"));
        cmd
    }

    #[cfg(windows)]
    fn command_null_stdin() -> Command {
        let mut cmd = Command::new("cmd.exe");
        cmd.args(["/C", "type NUL"]);
        cmd
    }
    ...
    #[cfg(windows)]
    fn command_large_stdout() -> Command {
        let mut cmd = Command::new("powershell.exe");
        cmd.args([
            "-NoProfile",
            "-Command",
            "$s='a'*80; 1..140000 | ForEach-Object { [Console]::Out.WriteLine($s) }",
        ]);
        cmd
    }
    ...
    #[cfg(windows)]
    fn command_large_stderr() -> Command {
        let mut cmd = Command::new("powershell.exe");
        cmd.args([
            "-NoProfile",
            "-Command",
            "$s='a'*80; 1..140000 | ForEach-Object { [Console]::Error.WriteLine($s) }",
        ]);
        cmd
    }
    ...
    fn test_run_streaming_passthrough_echo() {
        let mut cmd = command_stdout("hello");
    ...
    fn test_run_streaming_raw_cap_at_10mb() {
        // nosemgrep: interpreter-execution
        let mut cmd = command_large_stdout();
    ...
    fn test_run_streaming_stderr_cap_at_10mb() {
        // nosemgrep: interpreter-execution
        let mut cmd = command_large_stderr();
        let result = run_streaming(&mut cmd, StdinMode::Null, FilterMode::CaptureOnly).unwrap();
    ...
    fn test_run_streaming_null_stdin_cat() {
        let mut cmd = command_null_stdin();
        let result = run_streaming(&mut cmd, StdinMode::Null, FilterMode::Passthrough).unwrap();
        assert_eq!(result.exit_code, 0);
    }
    ...
    fn test_run_streaming_raw_contains_stdout() {
        let mut cmd = command_stdout("test_output_xyz");
        let result = run_streaming(&mut cmd, StdinMode::Null, FilterMode::CaptureOnly).unwrap();
    ...
    fn test_run_streaming_capture_only_filtered_equals_raw() {
        let mut cmd = command_stdout("check_equality");
        let result = run_streaming(&mut cmd, StdinMode::Null, FilterMode::CaptureOnly).unwrap();
    ...
    fn test_exec_capture_success() {
        let mut cmd = command_stdout("hello_capture");
